...
test('homepage chrome links its four service cards to real individual pages', () => { const s = homepageSource(); for (const slug of ['operations-support','customer-support','administrative-support','reporting-quality-assurance']) assert.ok(s.includes(slug), slug); assert.ok(/href=\{serviceLinks\[service\.slug\]\}/.test(s)); });
