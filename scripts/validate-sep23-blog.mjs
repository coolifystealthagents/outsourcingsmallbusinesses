...
const batch = fs.readFileSync(new URL('app/sep23-blog-batch.ts', root), 'utf8');
...
const manifest = JSON.parse(fs.readFileSync(new URL('.paperclip/daily-content/2026-09-23/blog.json', root), 'utf8'));
const topicSource = batch.split('export const septemberTwentyThreeSources')[0];
...
const trackedFiles = fs.readdirSync(new URL('app/', root)).filter((name) => /(?:blog-batch|content)\.ts$/.test(name) && name !== 'sep23-blog-batch.ts');
const older = trackedFiles.map((name) => fs.readFileSync(new URL(`app/${name}`, root), 'utf8')).join('\n') + fs.readFileSync(new URL('app/data.ts', root), 'utf8').split("import { septemberTwentyThreeBlogBatch")[0];
...
for (const marker of ['septemberTwentyThreeBlogBatch','2026-09-23','datePublished','canonical','article:published_time']) if (!(batch + data + page + sitemap).includes(marker)) throw new Error(`Missing integration marker: ${marker}`);
...
console.log(`Validated 12 new September 23 blog routes with ${words} substantive template words each.`);
