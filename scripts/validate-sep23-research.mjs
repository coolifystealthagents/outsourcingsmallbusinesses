...
const manifest = JSON.parse(fs.readFileSync('.paperclip/daily-content/2026-09-23/research.json', 'utf8'));
...
const posts = module.exports.septemberTwentyThreeResearchBatch;

if (manifest.family !== 'research' || manifest.publicationDate !== '2026-09-23' || manifest.requiredCount !== 5 || posts.length !== 5) fail('batch contract mismatch');
const inventory = fs.readdirSync('app').filter((name) => name.endsWith('.ts') && name !== 'sep23-research.ts').map((name) => fs.readFileSync(`app/${name}`, 'utf8')).join('\n');
...
const result = posts.map((post) => {
  ...
  if (words < 1200 || post.published !== '2026-09-23' || post.referenceSources.length < 3) fail(`editorial contract ${post.slug}`);
...
console.log(JSON.stringify({ count: 5, publicationDate: '2026-09-23', minimumWords: Math.min(...result.map((entry) => entry.words)), result }, null, 2));
