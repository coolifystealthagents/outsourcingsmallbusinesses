...
function AugustEighteenArticle({ post, index }: { post: (typeof blogPosts)[number]; index: number }) {
  ...
  return <><JsonLd data={schema} /><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-08-18"><p className="eyebrow">Small business operations guide</p><h1>{sourcePost.title}</h1><p className="lead">{sourcePost.excerpt}</p><time dateTime="2026-08-18">August 18, 2026</time><section><h2>Quick answer</h2><p>{sourcePost.opening}</p><ul>{sourcePost.checklist.map((item) => <li key={item}>{item}</li>)}</ul></section>{paragraphs.map((paragraph, i) => <section key={i}><h2>{['Start with a source record','Define the work unit','Separate facts from interpretation','Make the handoff reviewable','Measure the lane honestly'][i % 5]}</h2><p>{paragraph}</p></section>)}<section><h2>Keep the boundary clear</h2><p>{sourcePost.watchout}</p><p>{sourcePost.measure}</p><p>{sourcePost.lens}</p></section></article><CTA /></>;
}
