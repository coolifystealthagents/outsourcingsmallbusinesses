...
const septemberSevenBySlug = new Map<string, { post: (typeof septemberSevenBlogBatch)[number]; index: number }>(septemberSevenBlogBatch.map((post, index) => [post.slug, { post, index }]));
const septemberEightBySlug = new Map<string, { post: (typeof septemberEightBlogBatch)[number]; index: number }>(septemberEightBlogBatch.map((post, index) => [post.slug, { post, index }]));
const septemberNineBySlug = new Map<string, { post: (typeof septemberNineBlogBatch)[number]; index: number }>(septemberNineBlogBatch.map((post, index) => [post.slug, { post, index }]));
const septemberFourteenBySlug = new Map<string, { post: (typeof septemberFourteenBlogBatch)[number]; index: number }>(septemberFourteenBlogBatch.map((post, index) => [post.slug, { post, index }]));
const septemberTenBySlug = new Map<string, { post: (typeof septemberTenBlogBatch)[number]; index: number }>(septemberTenBlogBatch.map((post, index) => [post.slug, { post, index }]));
const septemberEighteenBySlug = new Map<string, { post: (typeof septemberEighteenBlogBatch)[number]; index: number }>(septemberEighteenBlogBatch.map((post, index) => [post.slug, { post, index }]));
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      siteName: site.brand,
      type: 'article',
      publishedTime: 'publicationDate' in post ? post.publicationDate : undefined,
      images: 'imagePath' in post && post.imagePath ? [post.imagePath] : undefined
    }
  };
}
...
function SeptemberSevenArticle({ index }: { index: number }) {
  const post = septemberSevenBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const related = septemberSevenBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,dateModified:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-07"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-07">September 7, 2026</time><figure><img src={post.imagePath} alt={`${post.lane} planning workspace`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Related September 7 guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}

function SeptemberEightArticle({ index }: { index: number }) {
  const post = septemberEightBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const related = septemberEightBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,dateModified:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-08"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-08">September 8, 2026</time><figure><img src={post.imagePath} alt={`${post.lane} planning workspace`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Related September 8 guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}

function SeptemberNineArticle({ index }: { index: number }) {
  const post = septemberNineBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const related = septemberNineBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,dateModified:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-09"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-09">September 9, 2026</time><figure><img src={post.imagePath} alt={`${post.lane} planning workspace`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Related September 9 guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}

function SeptemberTenArticle({ index }: { index: number }) {
  const post = septemberTenBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const related = septemberTenBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,dateModified:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-10"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-10">September 10, 2026</time><figure><img src={post.imagePath} alt={`${post.lane} planning workspace`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Related September 10 guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}

function SeptemberFourteenArticle({ index }: { index: number }) {
  const post = septemberFourteenBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const related = septemberFourteenBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,dateModified:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-14"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-14">Published: September 14, 2026</time><figure><img src={post.imagePath} alt={`${post.lane} planning workspace`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Related September 14 guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}

function SeptemberEighteenArticle({ index }: { index: number }) {
  const post = septemberEighteenBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const related = septemberEighteenBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  ...
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-18"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing decision guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-18">Published: September 18, 2026</time><figure><img src={post.imagePath} alt={`Small business owner planning ${post.lane} with an outsourced specialist`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Sources and next steps</h2><ul>{post.sources.map(([label,sourceUrl])=><li key={sourceUrl}><a href={sourceUrl} target="_blank" rel="noreferrer">{label}</a></li>)}</ul><p>For a practical on-site next step, review our <a href="/services/operations-support">operations support service guide</a>.</p></section><section><h2>Related outsourcing decision guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}
function SeptemberTwentyTwoArticle({ index }: { index: number }) {
  const post = septemberTwentyTwoBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const related = septemberTwentyTwoBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,citation:post.sources.map(([,sourceUrl])=>sourceUrl),author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-22"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing workflow guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-22">Published: September 22, 2026</time><figure><img src={post.imagePath} alt={`Small business owner planning ${post.lane} with a Philippines-based specialist`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Sources and next steps</h2><ul>{post.sources.map(([label,sourceUrl])=><li key={sourceUrl}><a href={sourceUrl} target="_blank" rel="noreferrer">{label}</a></li>)}</ul><p>For a practical next step, review the <a href="/services">small-business outsourcing service guides</a> and bring a redacted work sample to the scoping call.</p></section><section><h2>Related workflow guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}
function SeptemberTwentyThreeArticle({ index }: { index: number }) {
  const post = septemberTwentyThreeBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const servicePath = post.service === 'admin-support' ? 'administrative-support' : post.service === 'reporting-and-qa' ? 'reporting-quality-assurance' : post.service;
  const related = septemberTwentyThreeBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,citation:post.sources.map(([,sourceUrl])=>sourceUrl),author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
  return <><JsonLd data={schema}/><article className="container guide-article strict-article" data-article-family="blog" data-batch="2026-09-23"><link rel="canonical" href={url}/><meta property="article:published_time" content={post.publicationDate}/><p className="eyebrow">Small business outsourcing industry guide</p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p><time dateTime="2026-09-23">Published: September 23, 2026</time><figure><img src={post.imagePath} alt={`Small business owner planning ${post.lane} with a Philippines-based specialist`}/></figure>{post.sections.map(([heading,body])=><section key={heading}><h2>{heading}</h2><p>{body}</p></section>)}<section><h2>Sources and next steps</h2><ul>{post.sources.map(([label,sourceUrl])=><li key={sourceUrl}><a href={sourceUrl} target="_blank" rel="noreferrer">{label}</a></li>)}</ul><p>Compare the workflow with our <a href={`/services/${servicePath}`}>relevant outsourcing service guide</a>, or browse <a href="/services">all service guides</a>.</p></section><section><h2>Related industry guides</h2><div className="fleet-card-grid">{related.map(p=><a className="fleet-card" href={`/blog/${p.slug}`} key={p.slug}><h3>{p.title}</h3><p>{p.excerpt}</p></a>)}</div></section></article><CTA/></>;
}
function SeptemberTwentyFourArticle({ index }: { index: number }) {
  const post = septemberTwentyFourBlogBatch[index];
  const url = `${baseUrl}/blog/${post.slug}`;
  const servicePath = post.service === 'admin-support' ? 'administrative-support' : post.service === 'reporting-and-qa' ? 'reporting-quality-assurance' : post.service;
  const related = septemberTwentyFourBlogBatch.filter((_,i)=>i!==index).slice(index%9,index%9+3);
  const schema = {'@context':'https://schema.org','@type':'BlogPosting',headline:post.title,description:post.excerpt,url,datePublished:post.publicationDate,mainEntityOfPage:url,image:`${baseUrl}${post.imagePath}`,citation:post.sources.map(([,sourceUrl])=>sourceUrl),author:{'@type':'Organization',name:site.brand},publisher:{'@type':'Organization',name:site.brand,url:baseUrl}};
...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const septemberFourteen = septemberFourteenBySlug.get(slug);
  ...
  const septemberSeven = septemberSevenBySlug.get(slug);
  const septemberEight = septemberEightBySlug.get(slug);
  const septemberNine = septemberNineBySlug.get(slug);
  const septemberTen = septemberTenBySlug.get(slug);
  const septemberEighteen = septemberEighteenBySlug.get(slug);
  const septemberTwentyTwo = septemberTwentyTwoBySlug.get(slug);
